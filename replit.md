...
- **Bilingual attribute VALUES** — In `ProductFormPage`, text/number attribute fields now show two adjacent inputs (Arabic RTL + English LTR); English values stored as `{field_key}_en` in the same `attributes` JSONB field (no backend/migration change needed). Select-type dropdowns show `Arabic / English` combined option labels. In `ProductDetailPage`, `_en` keys are filtered from the attribute list and each value is shown as `قيمة عربية / English value` inline. `FlipbookPage` and count badges also filter `_en` keys. `AttributeSchemaItem` type updated with `options_en?` and `unit_en?`.
