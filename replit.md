...
- **Bilingual attribute VALUES** — In `ProductFormPage`, text/number attribute fields now show two adjacent inputs (Arabic RTL + English LTR); English values stored as `{field_key}_en` in the same `attributes` JSONB field (no backend/migration change needed). Select-type dropdowns show `Arabic / English` combined option labels. In `ProductDetailPage` and `FlipbookPage`, attribute keys are deduped to base keys (stripping any `_en` suffix), and ONE value is displayed per attribute: prefer the Arabic value when present, fall back to the English value if only the English one was entered. The count badge counts unique base keys. `AttributeSchemaItem` type updated with `options_en?` and `unit_en?`.
...
- **Our Projects (مشاريعنا)** module — backend `apps.projects` (Project + ProjectImage), frontend pages `/projects` (list grid) and `/projects/{new,:id/edit}` (form with two-step UX: save metadata first, then upload images). Sidebar entry "مشاريعنا" and the `/projects*` routes are visible to every authenticated user (`RequireAuth`); Add/Edit/Delete buttons, the upload drop-zone and the image-management section are gated inside each page on `super_admin || dept_manager` — non-editors see a read-only banner and a read-only image gallery. Permissions (`apps/projects/permissions.py`): super_admin full write/delete; dept_manager may create, update AND delete only when at least one of the project's products falls inside their managed categories (per `User.get_managed_category_ids()`); a draft project with no products yet is editable only by its creator. R2 storage at `projects/{project_id}/{uuid}.{ext}` (max 10MB, JPG/PNG/WEBP). Endpoints: `GET/POST/PATCH/DELETE /api/v1/projects/`, `PATCH /projects/{id}/toggle-active/`, `POST /projects/{id}/images/`, `DELETE /projects/{id}/images/{image_id}/`, `PATCH /projects/{id}/images/reorder/`, `GET /projects/search-products/?q=`. Public, unauthenticated endpoint `GET /api/v1/products/{id}/projects/` returns active projects for that product (used by `ProductDetailPage` via `ProjectsForProduct.tsx` with lightbox; modal renders product chips that link back to `/products/:id`). Serializer requires ≥1 product per project and re-validates manager scope on every create/update; the form blocks submission client-side too. Frontend uses a typed `getApiErrorMessage` helper in `src/api/errors.ts` instead of `any` casts on Axios errors.
